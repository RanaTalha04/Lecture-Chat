...
async function responseData(response) {
  ...
  if (!body) return { detail: `The server returned HTTP ${response.status} with no response body.` };
  try { return JSON.parse(body); }
  catch { return { detail: `The server returned HTTP ${response.status}: ${body.slice(0, 220)}` }; }
}
...
async function refreshStatus() {
  const response = await fetch('/api/health'); const data = await responseData(response);
  if (!response.ok) throw new Error(data.detail || 'Could not load library status.');
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const data = await responseData(response); if (!response.ok) throw new Error(data.detail || 'Request failed');
...
document.querySelector('#reindex').addEventListener('click', async (event) => {
  ...
  try { const response = await fetch('/api/reindex', {method: 'POST'}); const data = await responseData(response); if (!response.ok) throw new Error(data.detail); await refreshStatus(); }
...
document.querySelector('#upload-form').addEventListener('submit', async (event) => {
  ...
  try {
    const response = await fetch('/api/documents', {method: 'POST', body}); const data = await responseData(response);
